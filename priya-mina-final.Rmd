...
avg_pm25_plot
```
These two plots allow us to take a closer look at each graph. 
Higher temps occur from April to July in 2022, and April to June in 2023. 
Lower PM 2.5 occurs from April to September in 2022, and July in 2023 which is interesting......
...
```
It lets us see the general shape that higher temps = lower pm2.5. And higher PM2.5 occurs during lower temps. Its not the most helpful but it does let us see the shape, kinda.
...
combined_plot
```
This is not very helpful imo. the scaling doesn't help at all.
...
```
This helps us see the lowest and highest temps and PM 2.5 concentrations. 
...
ggplot() + 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
It looks like humidity and PM 2.5 follow somewhat similar patterns. When PM2.5 rises during the winter, umidity also seems to rise during Fall through Winter. When PM 2.5 has a decreasing trend starting in 2021-10 and going to 2022-07, humidity also has a decreasing trend. 
...
## QUESTION FOR MINA: WHERE ARE THE HUMIDITY SENSORS LOCATED? is it getting humidity from the homes/stoves, or the actual air?
